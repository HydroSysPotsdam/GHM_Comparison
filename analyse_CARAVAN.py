...
df = pd.read_csv(data_path + "complete_table.csv", sep=',')
#df = df.dropna()

df.rename(columns={'gauge_lon': 'lon', 'gauge_lat': 'lat'}, inplace=True)
...
# scatter plot
df.rename(columns={'Pmean': 'Precipitation', 'netrad_median': 'Net radiation',
               'evap': 'Actual ET', 'qr': 'Groundwater recharge', 'Qmean': 'Total runoff'}, inplace=True)
df["dummy"] = ""
df = df.loc[np.logical_and(df["area"]>250,df["area"]<25000)] # approx. between 10 and 1000% of a grid cell
#df = df.loc[(df["flow_perc_complete"]>90)]
...
df["Precipitation"] = df["Precipitation"]*365.25
df["Total runoff"] = df["Total runoff"]*365.25
...
g.savefig(results_path + x_name + '_' + y_name + "_scatterplot_CARAVAN.png", dpi=600, bbox_inches='tight')
plt.close()
...
for axes in g.axes.ravel():
    axes.legend(loc=(.0, .715), handletextpad=0.0, frameon=False, fontsize=9, labelspacing=0)
g.savefig(results_path + x_name + '_' + y_name + "_regressionplot_CARAVAN.png", dpi=600, bbox_inches='tight')
plt.close()
...
# save data to df
df.to_csv("data/" + "CARAVAN_prepared.csv", index=False)
